...
source("pseudolikapp.R")

MIMIC <- read_csv("MIMICchenchi.csv",
...
optim(
  c(beta_mle, sd_mle),
  pseudo_loglikelihood_app,
  df_MNAR = df_MNAR,
  control = list(maxit = 3000),
  hessian = T
) -> opt_pseudo
opt_pseudo$par - qnorm(0.975)*sqrt(diag(solve(opt_pseudo$hessian)))
opt_pseudo$par + qnorm(0.975)*sqrt(diag(solve(opt_pseudo$hessian)))
